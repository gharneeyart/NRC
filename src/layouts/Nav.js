...
export default function Nav() {
  ...
  // Function to handle nav link clicks (closes mobile menu)
          ...
          <div className={`lg:static container text-white text-[18.6px] flex flex-col gap-[3.375rem] top-24 rounded-sm lg:flex ${menuOpen ? 'w-full block opacity-100 bg-white right-0 p-8' : 'hidden opacity-0 top-[100%] py-1'} lg:block absolute lg:opacity-100 lg:flex-row lg:w-auto justify-between font-semibold h-screen lg:h-full z-50 lg:gap-[3rem] xl:gap-[12rem] 2xl:gap-[20rem]`}>
            <div className="flex flex-col lg:flex-row items-center gap-[1.625rem] xl:gap-[2rem] text-[18.6px] text-[#263238] lg:text-[#01320A] font-sans" ref={navRef}>
              <Link href="/" onClick={handleNavLinkClick}>
                <h1 className={`hover:text-[#006B14] ${isActive('/') ? 'text-[#006B14]' : ''}`}>Home</h1>
              </Link>
              <Link href="/about" onClick={handleNavLinkClick}>
                <h1 className={`hover:text-[#006B14] ${isActive('/about') ? 'text-[#006B14]' : ''}`}>About</h1>
              </Link>
              <Link href="/timetables" onClick={handleNavLinkClick}>
                <h1 className={`hover:text-[#006B14] ${isActive('/timetables') ? 'text-[#006B14]' : ''}`}>Time Tables</h1>
              </Link>
              <Link href="/faq" onClick={handleNavLinkClick}>
                <h1 className={`hover:text-[#006B14] ${isActive('/faq') ? 'text-[#006B14]' : ''}`}>FAQs</h1>
              </Link>
              <Link href="/contact" onClick={handleNavLinkClick}>
                <h1 className={`hover:text-[#006B14] ${isActive('/contact') ? 'text-[#006B14]' : ''}`}>Contact Us</h1>
              </Link>
              {auth.user != null ? (
                <div className="bg-white mx-auto items-center justify-center relative block lg:hidden">
                  ...
                  {dropOpen && (
                    <div className="w-[166px] py-[4px] absolute top-14 right-0 bg-white rounded-[10px] text-[#111014] text-[16px] drop-shadow shadow-[#00000026] cursor-pointer">
                      ...
                      <Link href="/ticket" className={`hover:text-[#006B14] ${isActive('/ticket') ? 'text-[#006B14]' : ''}`} onClick={handleNavLinkClick}>My Tickets</Link>
                ...
                <div className="w-full flex flex-col text-center gap-[1.75rem] lg:hidden">
                  <Link href="/auth/signup" className={`border ${isActive('/auth/signup') ? 'border-[#006B14] text-[#006B14]' : 'border-[#18A532] text-[#18A532]'} py-2 px-6 rounded-md w-full`} onClick={handleNavLinkClick}>Register</Link>
                  <Link href="/auth/login" onClick={handleNavLinkClick}>
                    <button className={`w-full ${isActive('/auth/login') ? 'bg-[#006B14]' : 'bg-[#18A532]'} text-white py-2 px-6 rounded-md`}>Sign in</button>
                  </Link>
                </div>
              )}
            </div>
            {auth.user != null ? (
              ...
              <div className="flex flex-col lg:flex-row text-center gap-[1.75rem] lg:gap-5 hidden lg:block space-x-5">
                <Link href="/auth/signup" className={`border ${isActive('/auth/signup') ? 'border-[#006B14] text-[#006B14]' : 'border-[#18A532] text-[#18A532]'} py-2 px-6 rounded-md`}>Register</Link>
                <Link href="/auth/login">
                  <button className={`${isActive('/auth/login') ? 'bg-[#006B14]' : 'bg-[#18A532]'} text-white py-2 px-6 rounded-md`}>Sign in</button>
                </Link>
              </div>
            )}
          </div>
