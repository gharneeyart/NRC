    ...
    return(
       <div className="w-full ">
        ...
        {Testimonials.map((testimonial) => (
          <div key={testimonial.id} className="   text-center flex flex-col lg:flex-row h-[350px] md:h-[241px] lg:h-[310px] w-[333.06px] lg:w-[1236.39px] mx-auto space-y-7 p-[23px] gap-[24px]">
           <div className="p-6 bg-white rounded-lg shadow-lg border border-gray-200 hover:shadow-2xl transition-all duration-300">
           
            <p className="text-left text-[#5F5F60] font-medium  mb-[25px]">{testimonial?.description}</p>
            
            <div className="flex items-center font-sans gap-[15.21px]">
              <Image src={testimonial.image} alt={testimonial.name}  />
              <p className="text-[16.3px] font-semibold">{testimonial?.name}</p>
            </div>
           </div>
          </div>
        ))}
